...
bool DataTask::fillVariables(std::vector<string> fileData)
{
    ...
    for (int i = 0; i < fileData.size(); i++) { //идем по первому столбцу контейнера
        switch (fileData[i][0]) {
            case 'C': { //заполняем вектор коэффициентов ЦФ
                ...
                for (j = 0; j < variablesVector.size(); j++) {
                    objectiveFunction[j] = variablesVector[j]; //заполняем вектор коэффициентов ЦФ
                }
            ...
            case 'A': {
                ...
                if (counter != numberOfVariables) {
                    cout << "Число столбцов системы ограничений не совпадает с числом переменных ЦФ" << endl;
                    return false;
                }

                constraintSystem = new double*[numberOfRows];
                for (int k = 0; k < numberOfRows; k++)
                    constraintSystem[k] = new double[counter];

                Row = 0;
                for (int k = 0; k < numberOfRows; k++) //заполняем матрицу системы ограничений
                    for (int l = 0; l < counter; l++) {
                        constraintSystem[k][l] = variablesVector[Row];
                        Row++;
                    }
            ...
            case 'B': { //заполняем вектор правой части системы ограничений
                while (j != fileData[i].size()) { //идем до конца строки
                    if (fileData[i][j] == '(') { //если встречена открывающаяся скобка
                        j++;
                        while (fileData[i][j] != ')') { //пока не встречена закрывающаяся скобка
